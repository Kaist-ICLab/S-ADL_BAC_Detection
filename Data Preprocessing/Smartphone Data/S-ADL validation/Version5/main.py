...
parser.add_argument('--with_all_device_event', action='store_true')
    ...
    for idx, row in df.iterrows():
        ...
        for req in reqs[2:]:
            ...
            if str(row['timestamp']) == str(req):
                if not args.with_all_device_event and row['datumType'] == 'DEVICE_EVENT':
                    if 'type' not in value.keys() or (value['type'] != "SCREEN_ON" and value['type'] != "SCREEN_OFF"):
                        continue
                info = dict()
