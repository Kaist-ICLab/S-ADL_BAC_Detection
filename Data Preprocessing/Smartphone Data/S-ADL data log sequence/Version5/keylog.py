...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        for req in reqs[2:]:
            ...
            if find:
                info = dict()
                for target in ['currentKey', 'timeTaken']:
                    if target in value.keys(): 
                        info[target] = value[target]

                current_key = info['currentKey']
                timeTaken = info['timeTaken']

                current_key = pd.DataFrame([[current_key]])
                timeTaken = pd.DataFrame([[timeTaken]])
                tmp = row.loc[['offsetTimestamp', 'timestamp']]
                tmp['currentKey'] = '\'' + current_key[0][0] + '\''
                tmp['timeTaken'] = timeTaken[0][0]
                tmp['offsetTimestamp'] = str(tmp['offsetTimestamp'])[:10] + 'T' + str(tmp['offsetTimestamp'])[11:] + '.000Z'
                tmp['timestamp'] = str(tmp['timestamp'])
                tmp['packageName'] = str(value['packageName'])
                find_rows = find_rows.append(tmp)
                break

    find_rows = find_rows[['currentKey', 'timeTaken', 'offsetTimestamp', 'timestamp', 'packageName']]
    print(find_rows.to_string())
    find_rows.to_excel('result.xlsx', encoding='utf-8-sig')
